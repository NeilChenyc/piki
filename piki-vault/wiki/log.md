...
## [2026-07-01] maintain | 全面展开微信背后的产品观 wiki（基于原始 PDF 重读）

- 来源/任务：用户要求"再读原 PDF，wiki 尽量维护全面"
- 重新提取 `raw/inbox/微信背后的产品观 (张小龙).pdf` → `raw/sources/1-1-72c6e0d0.md`（原有，已确认无变化）
- 修改路径：
  - `wiki/sources/微信背后的产品观.md` — 从概要层全面展开：新增用户篇（7 个人性特征）、需求篇（10 观点 + 完整方法框架 + 经典案例表）、设计篇（核心理念 + 方法论表格 + 重要原则 15+ 条）、气质篇（人文表达 + 未来感设计 + "有灵魂的产品"）、UI 篇（9 条原则）、提问环节精选。保留章节结构和金句。
  - `wiki/concepts/微信产品哲学.md` — 丰富：补充"需求来源优先级"体系、设计方法论核心原则表、UI 原则、关于创新的观点、结尾警示。
  - `wiki/entities/张小龙.md` — 补充：QQ 邮箱经历、设计方法论特点（6 项）、更多知名言论、一万小时定律观点。
  - `wiki/index.md` — 更新 3 条索引摘要，使其反映展开后的内容范围。
- 结果：微信产品哲学相关 3 页 wiki 全部从概要层提升为详细层，覆盖原始 PDF 全部 6 大章节 + 提问环节。
