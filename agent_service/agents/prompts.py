            ...
            content=_wrap_tag(
                ...
                "\n".join(
                    [
                        ...
                        "如果 action_context.action 是 run_lint，必须先用 Bash 调用 `$PIKI_RUNTIME_PYTHON -m agent_service.runtime.cli lint --vault .` 获取结构化检查结果，并把它作为后续分析与修复的起点。",
                        ...
                        "如果 action_context.action 是 ingest_file，必须处理 target_path 或 selected_paths 中的目标文件；需要先用 Bash 调用 `$PIKI_RUNTIME_PYTHON -m agent_service.runtime.cli extract-source --path <staged-path>` 生成 canonical source 内容，再继续按 AGENTS.md 编译 wiki。",
                        "运行 `agent_service.runtime.cli` 时必须使用环境变量 `$PIKI_RUNTIME_PYTHON` 指向的当前 Piki Runtime Python；不要直接使用系统 `python` 或 `python3`。",
                        "如果用户提供 selected_paths 并明确要求记录、摄入、整理或保存文档，应先用 Bash 提取结构化内容，再用 Write/Edit 完成 source 到 wiki 的维护流程。",
                        "`extract-source` 会返回 canonical_markdown、asset_path 和 source_path。用 canonical_markdown 通过 Write/Edit 写入 source_path；asset_path 只作为 canonical source 元数据保留，不要用 Bash 创建 raw/assets 或复制 staged 原文件。",
                        "不要再用 `cp`、`mv`、`mkdir`、重定向或其他 Bash 写操作去复制原文件或修改 vault/ raw/ wiki/ 内容。",
