...
from agent_service.models.lint_compat import (
    LintCompatIssue,
    LintCompatIssueKind,
    LintCompatResult,
    LintCompatSeverity,
...
def run_wiki_lint_compat(vault: Vault) -> LintCompatResult:
    ...
    issues: list[LintCompatIssue] = []

    for page in pages:
        if not page.content.startswith("---\n"):
            issues.append(
                _issue(
                    kind=LintCompatIssueKind.MISSING_FRONTMATTER,
                    severity=LintCompatSeverity.WARNING,
                    path=page.path,
        ...
        if not HEADING_PATTERN.search(_body_without_frontmatter(page.content)):
            issues.append(
                _issue(
                    kind=LintCompatIssueKind.MISSING_HEADING,
                    severity=LintCompatSeverity.WARNING,
                    path=page.path,
                    message="页面缺少一级标题。",
                )
        ...
        for target in page.wikilinks:
            ...
            if resolved is None:
                issues.append(
                    _issue(
                        kind=LintCompatIssueKind.BROKEN_LINK,
                        severity=LintCompatSeverity.ERROR,
                        path=page.path,
        ...
        if check_after and check_after <= date.today():
            issues.append(
                _issue(
                    kind=LintCompatIssueKind.STALE_PAGE,
                    severity=LintCompatSeverity.WARNING,
                    path=page.path,
        ...
        if len(_body_without_frontmatter(page.content).strip()) < 80 and page.path not in {"wiki/index.md", "wiki/log.md"}:
            issues.append(
                _issue(
                    kind=LintCompatIssueKind.THIN_PAGE,
                    severity=LintCompatSeverity.INFO,
                    path=page.path,
    ...
    for page in pages:
        ...
        if not incoming.get(page.path):
            issues.append(
                _issue(
                    kind=LintCompatIssueKind.ORPHAN_PAGE,
                    severity=LintCompatSeverity.INFO,
                    path=page.path,
                    message="页面没有入链。",
                )
        ...
        if not _index_mentions_page(index_text, page):
            ...
            issues.append(
                _issue(
                    kind=LintCompatIssueKind.MISSING_INDEX_ENTRY,
                    severity=LintCompatSeverity.WARNING,
                    path=page.path,
    ...
    for title, paths in title_map.items():
        if title and len(paths) > 1:
            for path in paths:
                issues.append(
                    _issue(
                        kind=LintCompatIssueKind.DUPLICATE_TITLE,
                        severity=LintCompatSeverity.WARNING,
                        path=path,
                        message=f"页面标题重复：{title}。",
                        details={"title": title, "paths": list(paths)},
                    )
                )
    ...
    for concept, count in concept_counts.items():
        ...
        issues.append(
            _issue(
                kind=LintCompatIssueKind.KNOWLEDGE_GAP,
                severity=LintCompatSeverity.INFO,
                path="wiki/index.md",
    ...
    return LintCompatResult(
        generated_at=utc_now_iso_compat(),
