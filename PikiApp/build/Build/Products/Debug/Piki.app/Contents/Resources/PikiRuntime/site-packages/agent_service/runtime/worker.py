...
class RuntimeWorker:
    ...
    def __post_init__(self) -> None:
        ...
        _diagnostic_log(
            "RuntimeWorker init "
            f"db_path={self.db_path} runtime_config_path={self.runtime_config_path} "
            f"staging_root={self.staging_root} enable_agent_runtime={self.enable_agent_runtime}"
        )
        ...
        _diagnostic_log(
            "RuntimeWorker ready "
            f"runner_available={self.runner.status.available} "
            f"agent_runtime_configured={self.config.agent_runtime_configured} "
            f"model={self.config.agent_model or '<unset>'} "
            f"base_url={self.config.anthropic_base_url or '<unset>'}"
        )

    def call(self, method: str, params: dict[str, Any]) -> dict[str, Any]:
        _diagnostic_log(f"call start method={method} params_keys={sorted(params.keys())}")
        if method == "health":
            result = self._health()
            _diagnostic_log(
                "call finish method=health "
                f"ok={result.get('ok')} runner_available={result.get('runner_available')} "
                f"agent_runtime_configured={result.get('agent_runtime_configured')}"
            )
            return result
        ...
        if method == "create_task":
            ...
            _diagnostic_log(
                "call finish method=create_task "
                f"task_id={result.task_id} status={result.status} async_mode={request.async_mode}"
            )
        ...
        if method == "task_events":
            ...
            _diagnostic_log(
                "call finish method=task_events "
                f"task_id={params['task_id']} events={len(result.get('events', []))}"
            )
            return result
    ...
    def _emit_notification(self, event: TaskEvent) -> None:
        _diagnostic_log(f"event emitted task_id={event.task_id} type={event.type}")
        self._notify_events()
