...
def test_system_context_wraps_agents_markdown_with_semantic_tags():
    ...
    assert [message.role for message in messages] == ["system", "system", "system", "system"]
    assert messages[0].name == "runtime_contract"
    assert "<runtime_contract>" in messages[0].content
    assert messages[1].name == "agents_md"
    assert "<writing_rules>" in messages[1].content
    assert "<ingest_workflow>" in messages[1].content
    assert "Keep wiki links stable." in messages[1].content
    assert messages[2].name == "purpose_context"
    assert "<purpose_context>" in messages[2].content
    assert messages[3].name == "wiki_index_context"
    assert "<wiki_index_context>" in messages[3].content
